...
then
	...
	if["$RESP" = "y" ]; then
		...
		if [ $ret -eq 0 ]; then
        PyInstall=true
        if [ sudo apt-get install python2.7 -y]; then
    ...
    if [sudo apt-get install python-wxgtk2.8 -y]; then
    ...
    # Compile and Build Python 2.7 under /opt
		if [sudo apt-get install --assume-yes avahi-utils libcups2 cups libcups2-dev cups-bsd cups-client libcupsimage2-dev libdbus-1-dev build-essential ghostscript openssl libjpeg-dev libsnmp-dev libtool libusb-1.0.0-dev wget python-imaging policykit-1 policykit-1-gnome gtk2-engines-pixbuf python-qt4 python-qt4-dbus python-dbus python-gobject python-dev python-notify2 python python-reportlab libsane libsane-dev sane-utils xsane zip]; then
      echo "Printer dependencies installed."
      if [sudo apt-get install hiplip -y]; then
        echo "hiplip succesfully installed."
        cd DIR
